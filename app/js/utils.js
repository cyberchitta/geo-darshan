...
export class MapOverlayGroup {
  constructor(group, mapManager, name) {
    this._group = group;
  ...
  static create(mapManager, name, visible = false) {
    const group = L.layerGroup();
    mapManager.addOverlayLayer(name, group, visible);
    return new MapOverlayGroup(group, mapManager, name);
  }

  get isVisible() {
    return this._mapManager.map.hasLayer(this._group);
  }
  addLayer(layer) {
  ...
  setOpacity(opacity) {
    this._group.eachLayer((l) => l.setOpacity(opacity));
  }
  on(event, handler) {
    this._group.on(event, handler);
  }
  off(event, handler) {
    this._group.off(event, handler);
  }
  destroy() {
